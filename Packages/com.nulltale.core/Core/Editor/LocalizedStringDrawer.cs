...
namespace CoreLib
{
    ...
    {
        private const int	k_LanguagePopupWidth = 60;
        private const int	k_SelectButtonWidth = 22;
        ...
        public override void OnGUI(Rect position, SerializedProperty property, GUIContent label)
        {
            ...
            if (core == null)
            {
                EditorGUI.LabelField(position, "Core not presented in scene");
                return;
            }
            ...
            var key = property.FindPropertyRelative(LocalizedString.k_KeyProperty);
            ...
            if (property.isExpanded)
            {
                ...
                var keyValue = EditorGUI.TextField(
                    new Rect(position.x + EditorGUIUtility.labelWidth, position.y, position.width - EditorGUIUtility.labelWidth - k_LanguagePopupWidth - k_SelectButtonWidth, EditorGUIUtility.singleLineHeight),
                    "", key.stringValue);
                ...
                // select key button
                if (GUI.Button(new Rect(position.x + position.width - k_LanguagePopupWidth - k_SelectButtonWidth, position.y, k_SelectButtonWidth, EditorGUIUtility.singleLineHeight),
                    "*"))
                    selectKey();

                // current language
                var languages = localizationManager.GetLanguages();
                var selectedLanguage = EditorGUI.Popup(new Rect(position.x + position.width - k_LanguagePopupWidth, position.y, k_LanguagePopupWidth, EditorGUIUtility.singleLineHeight),
            ...
            void updateValue(Rect textRect)
            {
                ...
                else
                {
                    ...
                    var localizationValue = EditorGUI.TextArea(textRect, LocalizationManager.k_DefaultKeyValue);

                    // not default value, initialize key
                    if (localizationValue != LocalizationManager.k_DefaultKeyValue)
